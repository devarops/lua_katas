...
describe("Find The Parity Outlier", function()
  ...
  of even integers except for a single integer N. Write a method that takes the array as any
  argument and returns this "outlier" N.]], function()
        assert.are.same(3, solution.FindOutlier.find({ 2, 6, 8, -10, 3 }))
        assert.are.same(206847684, solution.FindOutlier.find({ 206847684, 1056521, 7, 17, 1901, 21104421, 7, 1, 35521, 1, 7781 }))
        assert.are.same(0, solution.FindOutlier.find({ 2147483647, 0, 1 }))
    end)
end)
...
describe("Mumbling", function()
    ...
    it("The parameter of accum is a string which includes only letters from a..z and A..Z.", function()
